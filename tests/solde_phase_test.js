...
function refs(cs){
  ...
  const M=parts.map(keys=>{let w=0,p=0,nb=0,mm=0,re=0;keys.forEach((kk,ki)=>{const r=slice[kk];w+=r.waste;p+=r.pieces;nb+=r.nb;re+=r.pe;if(ki<keys.length-1)mm=Math.max(mm,r.pe);});return {keys,w,p,nb,ph:keys.length,mm,perte:w-re};});
  ...
  return {oldMid:opt.mm,oldPerte:opt.perte,domMid:dom.mm,domPerte:dom.perte};
}

const REELS=[
  { nom:'PRIMA (8 laizes)', midAttendu:1840, nbAttendu:36, useful:2080, blade:0, rows:[{width:20,qty:165},{width:30,qty:105},{width:40,qty:120},{width:50,qty:400},{width:60,qty:300},{width:70,qty:102},{width:80,qty:150},{width:100,qty:26}] },
...
for(let t=0;t<1200;t++){
  ...
  if(Math.abs(r.midMax-ref.domMid)>1e-9||Math.abs(r.perte-ref.domPerte)>1e-9) mism++;
}
...
console.log(fail?('\n💥 '+fail+' échec(s)'):'\n🏆 L220 DOMINANT VALIDÉ : grosse chute en FIN, perte JAMAIS aggravée, rendement + conservation intacts (1200 cas)');
