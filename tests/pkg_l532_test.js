...
const r2={'Cougnaud':_tri(PKG_CLIENTS['Cougnaud']),'BOUVET':_tri(PKG_CLIENTS['BOUVET']),'Suys':PKG_CLIENTS['Suys']}; _L532_PKG_FORCE.forEach(function(k){ if(!r2[k]) r2[k]=_tri(PKG_CLIENTS[k]); });   /* [L534] toute cle forcee ajoutee depuis (VEKA) fait partie d une table « a jour » */ ok(MERGE(r2)===r2,'table partagee DEJA a jour, cles dans l ordre de Firestore (triees) → rendue telle quelle (comparaison champ par champ)');
