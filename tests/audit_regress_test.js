...
has(/<i>mm de lame \/ coupe<\/i>/,'L514 : tuile « mm de lame / coupe » sur tous les plans (mono-réf compris)');
has(/body\.show-blade \.row3 \.field:has\(\[data-rb="blade"\]\)\{display:block\}/,'L514 : CSS de révélation du champ');
console.log('── L514b · vérification adverse (5) + audit familles (8) ──');
has(/\{ const chk=_l514LameCheck\(machine,num\);   \/\* \[L514c\] rejouée ICI sur le cache RAFRAÎCHI/,'L514b/c : la vérification vit DANS lameInstall et est rejouée sur le cache frais pour TOUS les chemins');
{ const _n=(src.match(/armed:true/g)||[]).length; console.log((_n===2?'✅ ':'❌ ')+'L514c : exactement 2 chemins déjà confirmés passent armed (✓ Validé, envoi) ('+_n+')'); if(_n!==2) fail++; }
absent(/skipCheck/,'L514c : plus aucun chemin ne SAUTE la vérification');
has(/return \{status:'indispo'\};/,'L514b : registre non chargé → « indispo » (jamais « inconnue »)');
has(/\{status:'jetee',quand:key\(byM\[mkn\]\)\}:\(byM\[mkn\]\.categorie==='affutage'\?\{status:'affutage',quand:key\(byM\[mkn\]\)\}:\{status:'ok'\}\)/,'L514b/c : connue sur CETTE machine → ok (n° par machine), sauf jetée à la benne ou partie chez l affûteur');
has(/^function _l514LameMsg\(mk,num,chk\)\{/m,'L514b : UN message par état, partagé par tous les écrans');
...
has(/^function _l514NetTyped\(block\)\{ try\{ if\(!block\) return false; const mo=parseNum/m,'L514c/d : « laize nette saisie » = bords 0 + mère > 0, SANS marqueur DOM (un booléen collant survivait au ↺ Nouveau plan)');
absent(/dataset\.typed/,'L514d : plus de marqueur de frappe collant');
has(/const _netTyped=_l514NetTyped\(block\);/,'L514c : onRefMachineChange applique les bords par défaut sauf laize nette saisie');
has(/const _net=_l514NetTyped\(block\); const e=block\.querySelector\('\[data-rb="edge"\]'\); if\(e&&!_net\) e\.value=d\.edge;/,'L514c : même règle dans la propagation L511');
has(/if\(ok\)\{ document\.getElementById\('lameInstallModal'\)\.classList\.remove\('open'\); _lameInstallMachine=null; renderMaintenance\(\); \}/,'L514c : Données > Lame — la modale ne se ferme qu après une pose réussie (le 2e tap reste possible)');
has(/\{status:'affutage',quand:key\(byM\[mkn\]\)\}/,'L514c : lame partie chez l affûteur → 2e tap « de retour ? », pas une re-pose silencieuse');
has(/async function _l514LameUndo\(mkArg\)\{/,'L514c : le ↩ d une carte machine annule le mémo de CETTE machine');
has(/if\(li\.what==null\) window\._l514Undone=/,'L514c : seule une POSE annulée bloque sa re-détection à l envoi');
has(/if\(!\/\^Seed\/\.test\(String\(description\|\|''\)\)\) window\._l514LastTrace=/,'L514c : un seed d inventaire n arme pas de ↩');
...
has(/#ficheMain #fichePkgMore\.l533-on\{display:inline-block;align-self:flex-start\}/,'L533 → L535 : le bouton ne s affiche QUE quand du texte est cache, et la zone cliquable s arrete REELLEMENT au texte (align-self : #ficheMain est un flex column, inline-block seul etait blockifie — 974 px mesures au navigateur au lieu de 238)');
